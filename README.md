# Go application stack for Kubernetes on Wodby

Deploy Go applications on Kubernetes with Wodby.
...
- [Go service](https://github.com/wodby/service-go)
...
| --- | --- |
| Go<br>`go` | required; enabled by default; main service; links: `db` → `postgres`, `redis` → `valkey`, `sendmail` → `mailpit` |
...
Start from [Go boilerplate](https://github.com/wodby/go-boilerplate), or connect your own compatible source
repository.
